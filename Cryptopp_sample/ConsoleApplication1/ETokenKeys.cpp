...
std::string ETokenKeys::gerarChaves()
{
	...
	try
	{
		...
		//new TEST
		Integer intPub;
		intPub.Decode(pubKey.BytePtr(), pubKey.SizeInBytes());
		std::string ret;
		//intPub.DEREncode(CryptoPP::StringSink(ret));
		Base64Encoder base64encoder(new StringSink(ret));
		intPub.DEREncode(base64encoder);
		...
		cout << "pubKey.size() = " << pubKey.size() << endl;
		cout << "dh.PublicKeyLength() = " << dh.PublicKeyLength() << endl;

		Integer a;
		a.Decode(pubKey.BytePtr(), pubKey.SizeInBytes());
		cout << "PUBKEY (A): " << std::hex << a << endl;
		
		string base64Key;
		Base64Encoder b(new StringSink(base64Key));
		b.Put(pubKey.data(), pubKey.size());
		b.MessageEnd();
		cout << "chave = " << base64Key << "\n";

		CryptoPP::byte* pubData = pubKey.data();
