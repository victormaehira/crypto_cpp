...
void toFile(unsigned char *buffer, size_t len)
{
	FILE *file = std::fopen("C:\\ProgramData\\desktopID\\keystore.txt.back", "w");
	if (file == NULL)
	{
...
void toFileAgainPraFazerODiff(unsigned char *buffer, size_t len)
{
	FILE *file = std::fopen("C:\\ProgramData\\desktopID\\keystore2.txt", "w");
	if (file == NULL)
	{
		fprintf(stderr, "Error opening file!\n");
		exit(EXIT_FAILURE);
...
unsigned char * fromFile() {
	...
	FILE * file = fopen("C:\\ProgramData\\desktopID\\keystore.txt.back", "r+");
	//buffer = (unsigned char *)malloc(size);
	int bytes_read = fread(buffer, sizeof(unsigned char), 20, file);
	fclose(file);
	return buffer;
}